...
def args_init_static():
    ...
    parser.add_argument("--class_dim",         type=int,       default=2,       help="")
    parser.add_argument("--layers",            type=list,      default=[8,4,2],       help="")
...
def stoc_grad_ascent(data, labels, net):
    ...
    for i in range(len(data)):
        ...
        for j in range(net_length):
            ...
            output.append(ReLU(input[j+1] - values[j]))
        ...
        dv1 = (lr / temp1) * np.sum((weights[-1] * temp2), axis = 1, keepdims=True).T
